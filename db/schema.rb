# This file is auto-generated from the current state of the database. Instead
# of editing this file, please use the migrations feature of Active Record to
# incrementally modify your database, and then regenerate this schema definition.
#
# This file is the source Rails uses to define your schema when running `bin/rails
# db:schema:load`. When creating a new database, `bin/rails db:schema:load` tends to
# be faster and is potentially less error prone than running all of your
# migrations from scratch. Old migrations may fail to apply correctly if those
# migrations use external dependencies or application code.
#
# It's strongly recommended that you check this file into your version control system.

ActiveRecord::Schema[8.1].define(version: 2026_02_13_175130) do
  # These are extensions that must be enabled in order to support this database
  enable_extension "pg_catalog.plpgsql"

  create_table "mood_caches", force: :cascade do |t|
    t.integer "access_count"
    t.datetime "created_at", null: false
    t.datetime "last_accessed_at"
    t.string "mood_key"
    t.jsonb "sentiment_data"
    t.jsonb "songs_data"
    t.datetime "updated_at", null: false
    t.integer "user_id"
  end

  create_table "songs", force: :cascade do |t|
    t.string "artist"
    t.datetime "created_at", null: false
    t.integer "duration_ms"
    t.string "image_url"
    t.string "spotify_id"
    t.string "spotify_uri"
    t.string "spotify_url"
    t.string "title"
    t.datetime "updated_at", null: false
    t.index ["spotify_id"], name: "index_songs_on_spotify_id", unique: true
  end

  create_table "user_songs", force: :cascade do |t|
    t.datetime "created_at", null: false
    t.string "mood", null: false
    t.text "mood_description"
    t.bigint "song_id", null: false
    t.datetime "updated_at", null: false
    t.bigint "user_id", null: false
    t.index ["song_id"], name: "index_user_songs_on_song_id"
    t.index ["user_id", "created_at"], name: "index_user_songs_on_user_id_and_created_at"
    t.index ["user_id"], name: "index_user_songs_on_user_id"
  end

  create_table "users", force: :cascade do |t|
    t.datetime "created_at", null: false
    t.string "email", null: false
    t.string "name"
    t.string "password_digest"
    t.datetime "updated_at", null: false
    t.index ["email"], name: "index_users_on_email", unique: true
  end

  add_foreign_key "user_songs", "songs"
  add_foreign_key "user_songs", "users"
end
